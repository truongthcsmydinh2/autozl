...
def run_device_automation(dev, device_index, delay, done_event, all_devices=None, stop_event=None):
    ...
    try:
        ...
        if stop_event and stop_event.is_set():
            print(f"[DEBUG] Global stop signal received for device {device_index}")
            return
        
        # Call the main flow function with all required parameters
        result = flow(dev, all_devices=all_devices, stop_event=stop_event)
            ...
            try:
                ...
                for device_index, dev in enumerate(connected_devices):
                    ...
                    thread = threading.Thread(
                        ...
                        args=(dev, device_index, delay, done_event, device_ips, stop_event),
...
def click_first_search_result(dev, preferred_text=None, debug=False):
    ...
    if debug: print(f"[DEBUG] click_first_search_result called with preferred_text={preferred_text}")
    try:
        ...
        if dev.click_by_resource_id("com.zing.zalo:id/btn_search_result", timeout=3, debug=False):
            ...
            if device_serial:
                ...
                if has_friend_btn:
                    ...
                    if debug: print("[DEBUG] 🔄 Starting friend request flow...")
                    friend_flow_success = dev.handle_friend_request_flow(debug=debug)
                    if debug: print(f"[DEBUG] 🔄 Friend request flow result: {friend_flow_success}")
                    
                    if friend_flow_success:
                        if debug: print("[DEBUG] ✅ Hoàn thành flow kết bạn → tiếp tục flow chính")
                    else:
                        if debug: print("[DEBUG] ❌ Flow kết bạn thất bại - RETURNING FALSE")
                        return False
                else:
                ...
                if dev.element_exists(resourceId="com.zing.zalo:id/btn_send_friend_request", timeout=3):
                    ...
                    # Thực hiện flow kết bạn ngay tại đây
                    if debug: print("[DEBUG] 🔄 Starting friend request flow (fallback)...")
                    friend_flow_success = dev.handle_friend_request_flow(debug=debug)
                    if debug: print(f"[DEBUG] 🔄 Friend request flow (fallback) result: {friend_flow_success}")
                    
                    if friend_flow_success:
                        if debug: print("[DEBUG] ✅ Hoàn thành flow kết bạn (fallback) → tiếp tục flow chính")
                    else:
                        if debug: print("[DEBUG] ❌ Flow kết bạn thất bại (fallback) - RETURNING FALSE")
                        return False
                else:
        ...
        if debug: print(f"[DEBUG] 🔄 Trying Method 2: Click by preferred text ({preferred_text})")
        ...
        if debug: print("[DEBUG] 🔄 Trying Method 3: Click first item in message list")
        if dev.element_exists(resourceId=RID_MSG_LIST):
        ...
        if debug: print("[DEBUG] 🔄 Trying Method 4: Click first clickable item")
        ...
        if debug: print("[DEBUG] 🔄 Trying Method 5: Fallback coordinates")
        dev.tap(540, 960)
        if debug: print("[DEBUG] ✅ Used fallback tap")
        return True
        
    except Exception as e:
        if debug: print(f"[DEBUG] ❌ EXCEPTION in click_first_search_result: {e}")
        if debug: print(f"[DEBUG] ❌ Exception type: {type(e).__name__}")
        import traceback
...
def send_message_human_like(dev, message, debug=False, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Kiểm tra tin nhắn đã gửi trước khi retry (tránh duplicate)
            if attempt > 0:
                if debug: print(f"[DEBUG] 🔍 Checking if message already sent before retry...")
                if verify_message_sent(dev, message, timeout=3, debug=debug):
                    if debug: print(f"[DEBUG] ✅ Message already sent successfully, skipping retry")
                    return True
...
def load_conversation_from_file(group_id):
    """Load cuộc hội thoại từ file conversations.json (ưu tiên) hoặc conversation_data.json (fallback)"""
    import json
    import os
    
    # Ưu tiên load từ conversations.json với group_id
    try:
        if os.path.exists('conversations.json'):
            with open('conversations.json', 'r', encoding='utf-8') as f:
                conversations_data = json.load(f)
            
            # Tìm conversation theo group_id
            for conversation in conversations_data:
                if conversation.get('group_id') == group_id:
                    messages = conversation.get('messages', [])
                    
                    # Convert format từ device_role sang device_number để tương thích
                    converted_messages = []
                    ...
                    print(f"[DEBUG] Loaded conversation for group {group_id} from conversations.json ({len(converted_messages)} messages)")
                    return converted_messages
                    
    except Exception as e:
        print(f"[DEBUG] Lỗi load từ conversations.json: {e}")
    
    # Fallback: load từ conversation_data.json với format cũ
    try:
        if os.path.exists('conversation_data.json'):
            ...
            if pair_key in conversations:
                print(f"[DEBUG] Fallback: Loaded conversation for group {group_id} from conversation_data.json")
                return conversations[pair_key].get('conversation', [])
                
    except Exception as e:
        print(f"[DEBUG] Lỗi load từ conversation_data.json: {e}")
...
def run_conversation(dev, device_role, debug=False, all_devices=None, stop_event=None, status_callback=None):
    ...
    if debug: print(f"[DEBUG] run_conversation started with device_role={device_role}, all_devices={all_devices}")
    
    # Lấy IP của device hiện tại
    device_ip = dev.device_id.split(":")[0] if ":" in dev.device_id else dev.device_id
    if debug: print(f"[DEBUG] device_ip extracted: {device_ip}")
    ...
    if debug: print(f"[DEBUG] Loaded conversation_data for group {group_id}: {len(conversation_data) if conversation_data else 0} items")
    if debug and conversation_data: print(f"[DEBUG] First conversation item: {conversation_data[0] if conversation_data else 'None'}")
    ...
    if not conversation:
        if debug: print(f"[DEBUG] ❌ No conversation data for group {group_id}")
        print(f"❌ Nhóm {group_id} - Không có cuộc hội thoại")
        return False
    
    if debug: print(f"[DEBUG] Converted conversation: {len(conversation)} messages")
    if debug and conversation: print(f"[DEBUG] First converted message: {conversation[0]}")
    ...
    if debug: print(f"[DEBUG] Starting conversation loop for {len(conversation)} messages")
    for msg in conversation:
        message_id = msg["message_id"]
        if debug: print(f"[DEBUG] Processing message_id {message_id}, sender={msg['sender']}, role_in_group={role_in_group}")
        
        # Kiểm tra stop signal trước xử lý mỗi message
        if stop_event and stop_event.is_set():
            if debug: print(f"[DEBUG] Stop signal received during conversation for {device_ip}")
        ...
        if msg["sender"] == role_in_group:
            ...
            # Thực hiện gửi tin nhắn với safe wrapper (giảm retry để tránh duplicate)
    ...
    if debug: print(f"[DEBUG] run_conversation completed successfully for device {device_ip}, group {group_id}")
    ...
    try:
        ...
        if os.path.exists(sync_file):
            os.remove(sync_file)
            if debug: print(f"[DEBUG] Cleaned up sync file: {sync_file}")
            print(f"🧹 Nhóm {group_id} - Đã cleanup sync file")
    except Exception as e:
        if debug: print(f"[DEBUG] Error cleaning sync file: {e}")
        pass
    
    if debug: print(f"[DEBUG] run_conversation returning True for device {device_ip}")
    return True
...
def verify_message_sent(dev, message_text, timeout=5, debug=False):
    """Xác minh tin nhắn đã được gửi thành công với logic cải thiện"""
    import time as time_module
    
    if debug:
        print(f"🔍 Xác minh tin nhắn đã gửi: '{message_text[:30]}...'")
    
    # Lưu trạng thái edit text trước khi gửi để so sánh
    initial_edit_text = ""
    try:
        edit_elem = dev.d(resourceId=RID_EDIT_TEXT)
        if edit_elem.exists:
            initial_edit_text = edit_elem.get_text() or ""
    except:
        pass
    
    start_time = time.time()
    message_found_count = 0  # Đếm số lần tìm thấy tin nhắn để tăng độ tin cậy
    
    while time.time() - start_time < timeout:
        try:
            # 1. Kiểm tra edit text đã clear (dấu hiệu mạnh nhất)
            edit_elem = dev.d(resourceId=RID_EDIT_TEXT)
            if edit_elem.exists:
                current_text = edit_elem.get_text() or ""
                # Nếu edit text đã clear hoàn toàn hoặc khác với text ban đầu
                if (not current_text.strip() and initial_edit_text.strip()) or \
                   (current_text != initial_edit_text and not current_text.strip()):
                    if debug:
                        print(f"✅ Edit text đã clear từ '{initial_edit_text}' thành '{current_text}' - tin nhắn đã gửi")
                    return True
            
            # 2. Tìm tin nhắn trong chat với nhiều cách khác nhau
            message_elements = [
                dev.d(text=message_text),
                dev.d(textContains=message_text[:30] if len(message_text) > 30 else message_text),
                dev.d(textContains=message_text[:20] if len(message_text) > 20 else message_text),
                dev.d(className="android.widget.TextView", textContains=message_text[:15] if len(message_text) > 15 else message_text),
                # Thêm tìm kiếm trong message container
                dev.d(resourceId="com.zing.zalo:id/message_content", textContains=message_text[:20] if len(message_text) > 20 else message_text),
                dev.d(resourceId="com.zing.zalo:id/tv_message", textContains=message_text[:20] if len(message_text) > 20 else message_text)
            ]
            
            for msg_elem in message_elements:
                if msg_elem.exists:
                    message_found_count += 1
                    if debug:
                        print(f"✅ Tìm thấy tin nhắn trong chat (lần {message_found_count})")
                    
                    # Nếu tìm thấy tin nhắn ít nhất 2 lần hoặc edit text đã clear
                    if message_found_count >= 2:
                        if debug:
                            print(f"✅ Xác nhận tin nhắn đã gửi thành công (tìm thấy {message_found_count} lần)")
                        return True
                    break
            
            # 3. Kiểm tra send button có còn disabled không
            send_btn = dev.d(resourceId=RID_SEND_BTN)
            if send_btn.exists and not send_btn.enabled:
                if debug:
                    print(f"✅ Send button disabled - có thể đang xử lý gửi tin nhắn")
            
            time_module.sleep(0.3)  # Giảm delay để kiểm tra nhanh hơn
    ...
    # Kiểm tra cuối cùng: nếu tìm thấy ít nhất 1 lần và edit text đã clear
    if message_found_count > 0:
        try:
            edit_elem = dev.d(resourceId=RID_EDIT_TEXT)
            if edit_elem.exists:
                current_text = edit_elem.get_text() or ""
                if not current_text.strip():
                    if debug:
                        print(f"✅ Tin nhắn tìm thấy {message_found_count} lần và edit text đã clear - xác nhận gửi thành công")
                    return True
        except:
            pass
    
    if debug:
        print(f"❌ Không thể xác minh tin nhắn sau {timeout}s (tìm thấy {message_found_count} lần)")
    return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                ...
                # Exponential backoff với delay dài hơn cho send message: 2s, 4s, 8s
                if "tin nhắn" in operation_name.lower() or "message" in operation_name.lower():
                    backoff_time = min(2 ** (attempt + 1), 10)  # 2s, 4s, 8s, 10s
                else:
                    backoff_time = min(2 ** attempt, 8)  # 1s, 2s, 4s, 8s
                if debug:
...
def flow(dev, all_devices=None, stop_event=None, status_callback=None):
    ...
    # Logic ghép cặp đơn giản
    if all_devices and len(all_devices) > 1:
        # Chuẩn hóa IP (bỏ port)
        normalized_devices = [d.split(':')[0] if ':' in d else d for d in all_devices]
        ...
        if 0 <= partner_index < len(sorted_devices):
            ...
            print(f"[DEBUG] Partner IP: {partner_ip}, Target phone: {target_phone}")
        else:
            target_phone = ""
    else:
        target_phone = ""
    ...
    if target_phone:
        ...
        print("• Không có số điện thoại partner - bỏ qua")
        return "SUCCESS"
    ...
    if click_first_search_result(dev, preferred_text=target_phone, debug=True):
        ...
        print(f"[DEBUG] Calling run_conversation with device_role=1, all_devices={all_devices}")
        conversation_result = run_conversation(dev, 1, debug=True, all_devices=all_devices, stop_event=stop_event, status_callback=status_callback)
        print(f"[DEBUG] run_conversation completed with result: {conversation_result}")
    else:
        print("❌ Không thể vào chat - click_first_search_result returned False")
        print(f"[DEBUG] click_first_search_result failed for target_phone: {target_phone}")
